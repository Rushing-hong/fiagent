...
> ⚠️ **工具状态**：本 skill 引用的 `alpha_zoo`、`alpha_bench` 工具和 `src/factors/registry.py` 代码库当前未在 fiagent 中实现。本 skill 提供因子库的概念参考（Kakushadze 101/GTJA 191/Qlib 158 是什么），但 Agent **不能调用**这些工具。可用 `factor_analysis` 工具对自行构建的因子 CSV 做 IC/IR 评估。
