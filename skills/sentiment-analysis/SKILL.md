...
> 🔧 **可用工具**：用 `get_limit_board` 获取涨停/跌停/炸板数据（涨停家数/跌停家数是核心情绪指标），用 `get_margin_trading` 获取融资余额变化，用 `get_northbound_flow` 获取北向资金流向，用 `get_fund_flow` 获取主力资金动向；行为因子方向用 `get_guba_sentiment`（东财股吧标题词典情绪）与 `calc_overnight_returns`（隔夜/日内收益分解），北向信号化用 `northbound_signal`。
...
> 加密恐贪指数 / VIX 等外盘指标仅作背景时用 `web_search`；本 skill 执行路径以 A 股工具为准。
...
### A 股行为因子工作流（Direction B）

A 股散户情绪与微观结构可组合为行为因子链（仅 A 股）：

```
1. get_guba_sentiment(code, pages=1-3)
   → 股吧标题词典情绪 score ∈ [-1,1]、bull_ratio / bear_ratio
   → 极端看多（score>0.5 且 bull_ratio>0.7）常作反向警示

2. calc_overnight_returns(codes, start, end, last_n=5)
   → overnight = open/prev_close-1；intraday = close/open-1
   → 隔夜偏负 + 日内偏正：常见于散户隔夜恐慌、盘中修复

3. get_fund_flow + northbound_signal
   → 主力/北向与舆情交叉验证，避免单一数据源

4. 短期反转提示（A 股）
   → 5-10 日超卖（RSI 极低、连续跌停后）+ 舆情极度悲观 → 行为学上的超反应反弹窗口
   → 与 behavioral-finance skill 中的 reversal 框架一致；勿与 20-60 日动量混用
```

**注意**：`get_guba_sentiment` 为标题关键词启发式，非 NLP 模型；机器人/营销帖会干扰，需与资金、涨跌停广度交叉验证。
