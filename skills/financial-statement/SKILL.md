...
> **优先调用 `check_red_flags` 工具自动扫描**（首批 6 条：盈利质量/应收/扣非/商誉/存货/存贷双高）。工具只给触发列表与数值，严重度与投资结论由你解读。下列 12 条作补充检查清单。
...
> **优先调用 `calc_dupont` 工具获取自动分解结果**（3 因子 + 可选 5 因子 + 连环替代贡献度）。金融股自动改 ROA×权益乘数。不要手算乘积与归因。
...
6. **数据来源**：优先 `get_financial_statements`（东财 F10，含 `normalized`）；行情侧 PE/PB 可用 `screen_market` / `screen_fundamental`，勿假定 tushare `extra_fields` 工作流
