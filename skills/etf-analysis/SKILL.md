...
- **ETF 行情**：`get_market_data(codes=["510300.SH"], ...)`（腾讯/东财）
- **同类 ETF 对比 / 筛选**：`screen_market` + `search_symbol`；深度筛选可配 `FIAGENT_IWENCAI_KEY` 后用 `iwencai_search`
- **板块暴露**：`get_sector_info(code="510300.SH")`
- **Tushare ETF 净值/规模**（可选）：见 `tushare` skill，需 `TUSHARE_TOKEN`
- 分析方法论见下文；第 7 节代码示例以 Tushare 为主，可按上表替换为 fiagent 工具
