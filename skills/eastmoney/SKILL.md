...
description: 东方财富免费免鉴权数据接口索引：资金流向、龙虎榜、融资融券、大宗交易、股东户数、限售解禁、板块、研报、新闻、财报、选股。通过 fiagent 内置工具调用，无需 token。
---
# Eastmoney（东方财富）

## 概述

东方财富对外开放了一批免费、免鉴权的行情与披露接口（push2 / push2his / datacenter-web / reportapi / search-api）。这些接口由 **fiagent 内置工具**封装，统一返回 `{"ok": true/false, ...}` JSON 信封。本技能是上述接口的**索引页**：端点 URL、入参、返回字段写在 `references/` 下。
...
screen_market(market="a", sort_by="change_pct", top_n=20)
```

底层实现在 `market/eastmoney.py`（secid 解析 + 限速 GET）。
