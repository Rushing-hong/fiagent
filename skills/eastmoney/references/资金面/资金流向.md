...
工具：`get_fund_flow`（`tools.stock_flow.FundFlowTool`）
描述：东方财富免费免鉴权资金流向序列，把每根 bar 的净流入拆成 主力/超大单/大单/中单/小单 五档，可取日线历史或当日分钟线。
市场：仅 A 股（`.SH` / `.SZ` / `.BJ`）。
...
名称 | 类型 | 必选 | 描述
---- | ---- | ---- | ----
codes | list[str] | Y | A 股 symbol 列表，如 `["600519.SH", "000001.SZ"]`。单个不可解析 symbol 按 per-symbol error 上报，不中断批次。
...
名称 | 取值 | 描述
---- | ---- | ----
secid | `<market>.<code>` | 由 symbol 解析（SH=1，SZ/BJ=0）
...
out = FundFlowTool().execute(
    {"codes": ["600519.SH"], "period": "daily", "days": 30}, None
)
print(out)
```
